<template>
    <app-layout title="Pemegang Hak">
        ...
        <jet-modal
            ...
            title="Tambah pemegang hak"
        >
            <template #content>
                Silakan masukkan data pemegang hak yang ingin ditambahkan.
        ...
        <jet-modal
            ...
            title="Edit pemegang hak"
        >
            <template #content>
                Silakan masukkan data pemegang hak yang ingin diubah.
                <jet-validation-errors class="mt-4" />
                <div class="mt-4">
                    <jet-input
                        type="text"
        ...
        <jet-alert-modal
            ...
            title="Hapus pemegang hak"
        >
            <template #content>
                Apakah Anda yakin ingin menghapus pemegang hak ini? Setelah
                pemegang hak dihapus, semua sumber daya dan datanya akan dihapus
        ...
        <jet-import-modal
            ...
            title="Impor data pemegang hak"
        >
            <template #content>
                <p>
                    Silakan unggah file data pemegang hak yang ingin di-impor.
        ...
        <jet-export-modal
            ...
            title="Ekspor data pemegang hak"
        >
            <template #content>
                <p>
...
export default defineComponent({
    ...
    methods: {
        store() {
            this.storeForm.post(route("holders.store"), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData();
                    this.closeStoreModal();
                    this.showSuccessNotification(
                        "Pemegang Hak berhasil ditambahkan",
                        "Sistem telah berhasil menyimpan data pemegang hak baru"
                    );
                },
                onError: () =>
                    this.showDangerNotification(
                        "Kesalahan telah terjadi",
                        "Sistem tidak dapat menyimpan data pemegang hak, mohon periksa ulang form"
                    ),
            });
        },
        update() {
            this.updateForm.put(route("holders.update", this.updateForm.id), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData();
                    this.closeUpdateModal();
                    this.showSuccessNotification(
                        "Pemegang Hak berhasil diedit",
                        "Sistem telah berhasil mengedit data pemegang hak"
                    );
                },
                onError: () =>
                    this.showDangerNotification(
                        "Kesalahan telah terjadi",
                        "Sistem tidak dapat mengubah data pemegang hak, mohon periksa ulang form"
                    ),
            });
        },
        destroy() {
            this.destroyForm.delete(
                route("holders.destroy", this.destroyForm.id),
                {
                    preserveScroll: true,
                    onSuccess: () => {
                        this.reloadData();
                        ...
                        this.showSuccessNotification(
                            "Pemegang Hak berhasil dihapus",
                            "Sistem telah berhasil menghapus data pemegang hak"
                        );
                    },
                    onError: () =>
                        this.showDangerNotification(
                            "Kesalahan telah terjadi",
                            "Sistem tidak dapat menghapus data pemegang hak"
                        ),
                }
            );
        },
        importFile() {
            this.importForm.post(route("holders.import"), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData();
                    ...
                    this.showSuccessNotification(
                        ...
                        "Sistem berhasil menjadwalkan permintaan impor data pemegang hak di latar belakang"
                    );
                },
                onError: () =>
                    this.showDangerNotification(
                        "Kesalahan telah terjadi",
                        "Sistem tidak dapat mengimpor data pemegang hak, mohon periksa kesalahan yang telah dideteksi"
                    ),
            });
        },
        exportFile() {
            window.open(route("holders.export"));
        ...
        confirmUpdate(holder) {
            this.updateForm.id = holder.id;
            this.updateForm.name = holder.name;
            setTimeout(() => (this.confirmingUpdate = true), 150);
            setTimeout(() => this.$refs.updateFuncloc.focus(), 300);
        },
        confirmDestroy(holder) {
            this.destroyForm.id = holder.id;
        ...
        reloadData() {
            this.$refs.table.reload("holders");
        },
    },
});
</script>
