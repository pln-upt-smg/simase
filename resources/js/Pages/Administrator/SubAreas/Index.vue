<template>
    <app-layout title="Sub Area">
        ...
        <jet-table
            ...
            <template #body>
                <tr v-for="subArea in subAreas.data" :key="subArea.id">
                    <td v-show="showColumn('name')">{{ subArea.name }}</td>
                    <td v-show="showColumn('sloc')">{{ subArea.sloc }}</td>
                    <td v-show="showColumn('action')" class="text-center">
                        <jet-dropdown name="Opsi">
                            <menu-item>
                                <button @click="confirmUpdate(subArea)"
                            ...
                            <menu-item>
                                <button @click="confirmDestroy(subArea)"
                                        class="text-gray-700 hover:bg-gray-100 group flex items-center px-4 py-2 text-sm w-full">
        ...
        <jet-modal :show="confirmingStore" @close="closeStoreModal" title="Tambah sub area">
            <template #content>
                Silakan masukkan data sub area yang ingin ditambahkan.
                <jet-validation-errors class="mt-4"/>
                <div class="mt-4">
                    <jet-input type="text" class="block w-full capitalize" placeholder="Nama Sub Area"
        ...
        <jet-modal :show="confirmingUpdate" @close="closeUpdateModal" title="Edit sub area">
            <template #content>
                Silakan masukkan data sub area yang ingin diubah.
                <jet-validation-errors class="mt-4"/>
                <div class="mt-4">
                    <jet-input type="text" class="block w-full capitalize" placeholder="Nama Sub Area"
        ...
        <jet-alert-modal :show="confirmingDestroy" @close="closeDestroyModal" title="Hapus sub area">
            <template #content>
                Apakah Anda yakin ingin menghapus sub area ini? Setelah sub area dihapus, semua sumber daya
        ...
        <jet-import-modal :show="confirmingImport" @close="closeImportModal" title="Impor data sub area">
            <template #content>
                <p>
                    Silakan unggah file data sub area yang ingin di-impor. Pastikan Anda sudah menggunakan template
        ...
        <jet-export-modal :show="confirmingExport" @close="closeExportModal" title="Ekspor data sub area">
            <template #content>
                <p>
                    Apakah Anda yakin ingin mengekspor semua data sub area? Proses ekspor dapat memakan waktu lama,
...
export default defineComponent({
    ...
    methods: {
        store() {
            this.storeForm.post(route('subareas.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData()
                    this.closeStoreModal()
                    this.showSuccessNotification('Sub Area berhasil ditambahkan', 'Sistem telah berhasil menyimpan data sub area baru')
                },
                onError: () => this.showDangerNotification('Kesalahan telah terjadi', 'Sistem tidak dapat menyimpan data sub area, mohon periksa ulang form')
            })
        },
        update() {
            this.updateForm.put(route('subareas.update', this.updateForm.id), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData()
                    this.closeUpdateModal()
                    this.showSuccessNotification('Sub Area berhasil diedit', 'Sistem telah berhasil mengedit data sub area')
                },
                onError: () => this.showDangerNotification('Kesalahan telah terjadi', 'Sistem tidak dapat mengubah data sub area, mohon periksa ulang form')
            })
        },
        destroy() {
            this.destroyForm.delete(route('subareas.destroy', this.destroyForm.id), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData()
                    this.closeDestroyModal()
                    this.showSuccessNotification('Sub Area berhasil dihapus', 'Sistem telah berhasil menghapus data sub area')
                },
                onError: () => this.showDangerNotification('Kesalahan telah terjadi', 'Sistem tidak dapat menghapus data sub area')
            })
        },
        importFile() {
            this.importForm.post(route('subareas.import'), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData()
                    this.closeImportModal()
                    this.showSuccessNotification('Permintaan impor data telah dijadwalkan', 'Sistem berhasil menjadwalkan permintaan impor data sub area di latar belakang')
                },
                onError: () => this.showDangerNotification('Kesalahan telah terjadi', 'Sistem tidak dapat mengimpor data sub area, mohon periksa kesalahan yang telah dideteksi')
            })
        },
        exportFile() {
            window.open(route('subareas.export'))
        ...
        confirmUpdate(subArea) {
            this.updateForm.id = subArea.id
            this.updateForm.name = subArea.name
            this.updateForm.sloc = subArea.sloc
            setTimeout(() => this.confirmingUpdate = true, 150)
            setTimeout(() => this.$refs.updateName.focus(), 300)
        },
        confirmDestroy(subArea) {
            this.destroyForm.id = subArea.id
        ...
        reloadData() {
            this.$refs.table.reload('subAreas')
        }
    }
})
</script>
