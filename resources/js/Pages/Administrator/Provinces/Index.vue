<template>
    <app-layout title="Provinsi">
        ...
        >
            <template #content>
                Silakan masukkan data provinsi yang ingin ditambahkan.
        ...
        >
            <template #content>
                Silakan masukkan data provinsi yang ingin diubah.
                <jet-validation-errors class="mt-4" />
                <div class="mt-4">
                    <jet-input
                        type="text"
        ...
        >
            <template #content>
                Apakah Anda yakin ingin menghapus provinsi ini? Setelah provinsi
        ...
        <jet-import-modal
            ...
            title="Impor data provinsi"
        >
            <template #content>
                <p>
                    Silakan unggah file data provinsi yang ingin di-impor.
...
export default defineComponent({
    ...
    methods: {
        store() {
            this.storeForm.post(route("provinces.store"), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData();
                    ...
                    this.showSuccessNotification(
                        ...
                        "Sistem telah berhasil menyimpan data provinsi baru"
                    );
                },
                onError: () =>
                    this.showDangerNotification(
                        "Kesalahan telah terjadi",
                        "Sistem tidak dapat menyimpan data provinsi, mohon periksa ulang form"
                    ),
            });
        },
        update() {
            this.updateForm.put(route("provinces.update", this.updateForm.id), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData();
                    this.closeUpdateModal();
                    this.showSuccessNotification(
                        "Provinsi berhasil diedit",
                        "Sistem telah berhasil mengedit data provinsi"
                    );
                },
                onError: () =>
                    this.showDangerNotification(
                        "Kesalahan telah terjadi",
                        "Sistem tidak dapat mengubah data provinsi, mohon periksa ulang form"
                    ),
            });
        },
        destroy() {
            this.destroyForm.delete(
                route("provinces.destroy", this.destroyForm.id),
                {
                    preserveScroll: true,
                    onSuccess: () => {
                        this.reloadData();
                        ...
                        this.showSuccessNotification(
                            ...
                            "Sistem telah berhasil menghapus data provinsi"
                        );
                    },
                    onError: () =>
                        this.showDangerNotification(
                            "Kesalahan telah terjadi",
                            "Sistem tidak dapat menghapus data provinsi"
                        ),
                }
            );
        },
        importFile() {
            this.importForm.post(route("provinces.import"), {
                preserveScroll: true,
                onSuccess: () => {
                    this.reloadData();
                    ...
                    this.showSuccessNotification(
                        ...
                        "Sistem berhasil menjadwalkan permintaan impor data provinsi di latar belakang"
                    );
                },
                onError: () =>
                    this.showDangerNotification(
                        "Kesalahan telah terjadi",
                        "Sistem tidak dapat mengimpor data provinsi, mohon periksa kesalahan yang telah dideteksi"
                    ),
            });
        },
        exportFile() {
            window.open(route("provinces.export"));
        ...
        confirmUpdate(province) {
            this.updateForm.id = province.id;
            this.updateForm.name = province.name;
            setTimeout(() => (this.confirmingUpdate = true), 150);
            setTimeout(() => this.$refs.updateFuncloc.focus(), 300);
        },
        confirmDestroy(province) {
            this.destroyForm.id = province.id;
        ...
        reloadData() {
            this.$refs.table.reload("provinces");
        },
    },
});
</script>
